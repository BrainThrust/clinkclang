...
```bash
python -m examples/basic/bare_minimum
```

# Swarm (experimental, educational)

> Showcase the handoff & routines patterns explored in the [Orchestrating Agents: Handoffs & Routines](https://cookbook.openai.com/examples/orchestrating_agents) cookbook.
...
Lightweight, highly controllable, and easily testable.
...
Swarm explores patterns that are lightweight, scalable, and highly customizable by design. Does not store state between calls.
...
> [!NOTE]
> Not related to Assistants in the Assistants API. They are named similarly for convenience, but are otherwise completely unrelated. Swarm is entirely powered by the Chat Completions API and is hence stateless between calls.
...
Evaluations are crucial to any project.

For reference, we have some examples for how to eval swarm in the `airline`, `weather_agent` and `triage_agent` quickstart examples. See the READMEs for more details.
